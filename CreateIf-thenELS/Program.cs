    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                PrintSampleParams("CreateIf-thenELS", loginParams, sampleParams);
                ...
                if (statusListener.WaitEvents() && statusListener.Connected)
                {
                    ...
                    double otoRate = offer.Bid + 2.0 * offer.PointSize;
                    double dRateELS = offer.Bid - 30.0 * offer.PointSize;
                    double dRateStop = offer.Bid - 50.0 * offer.PointSize;
                    double dRateLimit = offer.Bid + 50.0 * offer.PointSize;

                    Console.WriteLine("Bid={0}, Ask={1}, OTO={2}, ELS={3}, Stop={4}, Limit={5}", offer.Bid, offer.Ask, otoRate, dRateELS, dRateStop, dRateLimit);

                    O2GRequest request = CreateOTO_ELSRequest(session, offer.OfferID, account.AccountID, iAmount, otoRate, dRateELS, dRateStop, dRateLimit);
                    if (request == null)
                    {
