...
void ResponseListener::printOffers(IO2GSession *session, IO2GResponse *response, const char *sInstrument)
{
    ...
    if (offersResponseReader != NULL)
    {
		string symbol;
		ofstream xmlfile;
		xmlfile.open("rollover.xml");
		xmlfile << "<?xml version=\"1.0\"?>" << std::endl;
		xmlfile << "<rates>" << std::endl;
	
		ofstream txtfile;
		txtfile.open("rollover.txt");
        ...
        for (int i = 0; i < offersResponseReader->size(); ++i)
        {
            ...
            if (!sInstrument || strlen(sInstrument) == 0 || strcmp(offerRow->getInstrument(), sInstrument) == 0)
            {
				...
				xmlfile << "<rate>" << std::endl;
				xmlfile << "<symbol>" << symbol << "</symbol>" << std::endl;
				xmlfile << "<rolbuy>" << offerRow->getBuyInterest() << "</rolbuy>" << std::endl;
				xmlfile << "<rolsell>" << offerRow->getSellInterest() << "</rolsell>" << std::endl;
				xmlfile << "</rate>" << std::endl;

				txtfile << offer->getInstrument() << "," << offerRow->getSellInterest() << "," << offerRow->getBuyInterest() << std::endl;
            }
        }
		xmlfile << "</rates>" << std::endl;
		xmlfile.close();
		txtfile.close();
		std::cout << "Rollover rates saved to file rollover.xml" << std::endl;
    }
}
